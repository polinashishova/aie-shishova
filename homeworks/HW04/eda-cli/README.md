...
Новые параметры:
- `top_k_categories` - количество выводимых значений качественных признаков, по умолчанию 5;
- `title` - заголовок отчёта, по умолчанию 'EDA-Отчёт'.

Пример использования с новыми параметрами:

```bash
uv run eda-cli report data/example.csv --top-k-categories 2 --title 'отчет' --out-dir reports_example
```
...
### 5. Новый эндпоинт `POST /quality-flags-from-csv` – получение флагов качества по CSV-файлу

Эндпоинт, который принимает CSV-файл, внутри:
...
- вызывает функции из `eda_cli.core`:
  ...
  - `missing_table`,
  - `compute_quality_flags`;
- и возвращает детальные флаги качества.

**Запрос через Swagger:**
...
- `flags` - булевы флаги из `compute_quality_flags` в формате JSON.
