...
@app.post(
    ...
    tags=["quality"],
    summary="Получение детальных флагов качества по CSV-файлу"
)
async def quality_flags_from_csv(file: UploadFile = File(...)):
    """
    Эндпоинт, который принимает CSV-файл и возвращает детальные флаги качества.
    """
    ...
    print(f"[quality-flags-from-csv] filename={file.filename!r} flags={response_flags}")

    return {"flags": response_flags}