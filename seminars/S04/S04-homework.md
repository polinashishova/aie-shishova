...
2. Скопируйте эталонный файл `api.py` из материалов семинара S04 в свой проект HW04:
   ...
   - **не переписывать заново** эндпоинты `/health`, `/predict`, `/predict-from-csv`, реализованные на семинаре.
...
3. После копирования файла `api.py`:
   ...
   Проверьте, что:
      ...
      - `POST /predict` и `POST /predict-from-csv` работают так же, как в примере с семинара (через `/docs` или HTTP-клиент).

4. Далее (см. п. **2.3.2**) вам нужно **доработать** этот HTTP-сервис: добавить собственный эндпоинт, завязанный на доработки HW03. Реализовывать с нуля `/health`, `/predict` и `/predict-from-csv` в рамках HW04 не требуется.
...
- **не дублирует** уже реализованные в семинарном `api.py` эндпоинты `/health`, `/predict` или `/predict-from-csv`;
...
1. Эндпоинт:
   ...
   - принимает CSV-файл (аналогично `/predict-from-csv`);
...
- Написать простой скрипт (например, `scripts/client.py`), который:
  ...
  - отправляет несколько запросов к `/predict` и/или `/predict-from-csv` с разными параметрами/файлами;
...
- HTTP-сервис должен корректно отвечать на `GET /health`, `POST /predict`, `POST /predict-from-csv` и ваш новый эндпоинт.
...
4. Эндпоинт `POST /predict`:

   - принимает JSON по схеме `PredictRequest`;
   - возвращает JSON по схеме, совместимой с `PredictResponse` (включая `ok_for_model`, `quality_score`, `latency_ms`, `flags`);
   - логика предсказания зависит от входных параметров.
5. Эндпоинт `POST /predict-from-csv`:
...
6. Реализован **как минимум один дополнительный эндпоинт**, который:
   ...
   - не дублирует `/health`, `/predict`, `/predict-from-csv`;
...
- Фактом сдачи работы считается наличие к указанному дедлайну:
  ...
  - реализованных HTTP-эндпоинтов (`/health`, `/predict`, `/predict-from-csv` и дополнительного эндпоинта);
