...
- может реализовать минимальный HTTP-сервис поверх своего `eda-cli`:
  ...
  - эндпоинт `/predict` с JSON-запросом и эвристическим «предсказанием» качества;
  - эндпоинт `/predict-from-csv`, который использует функции `summarize_dataset`, `missing_table`, `compute_quality_flags` из ядра проекта;
...
4. **Эндпоинт `/predict`: контракт по агрегированным признакам**
   - проектирование pydantic-моделей `PredictRequest` и `PredictResponse`:
...
5. **Эндпоинт `/predict-from-csv`: связка с EDA-ядром S03**
   ...
   - формирование `PredictResponse` на основе `quality_score` и булевых флагов качества;
...
7. **Демонстрация запуска и тестирования сервиса**
   ...
   - использование Swagger UI (`/docs`) для отправки запросов на `/predict` и `/predict-from-csv`;
...
8. **Постановка домашнего задания HW04**
   ...
   - обзор критериев зачёта HW04 на уровне:
     ...
     - `/health`, `/predict`, `/predict-from-csv` работают корректно;
...
- **Практическая часть (live-coding + демонстрация проекта):**
  ...
  - реализация и запуск эндпоинтов `/health`, `/predict`, `/predict-from-csv`;
...
- умеет:
  ...
  - реализовать и проверить эндпоинты `/health`, `/predict`, `/predict-from-csv`;
...
- реализован HTTP-сервис качества датасетов с базовыми эндпоинтами `/health`, `/predict`, `/predict-from-csv`;
